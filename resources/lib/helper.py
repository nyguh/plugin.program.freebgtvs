# -*- coding: utf8 -*-
import os, sys, requests, re, xbmc
...
def update(name, dat, crash=None):
  payload = {}
  payload['an'] = name
  payload['av'] = addon.getAddonInfo('version')
  payload['ec'] = name
  payload['ea'] = 'free_bgtvs_service'
  payload['ev'] = '1'
...
class Playlist:
  ...
  def __init__(self, name = ''):
    if name != '':
      self.name = name
    xbmc.log("Creating playlist with name: " + self.name)
  
  def save(self, path):
    file_path = os.path.join(path, self.name)
    xbmc.log("Saving playlist to: %s " % file_path)
    if os.path.exists(path):
      file = open(file_path, 'w')
      file.write(self.to_string().encode('utf-8', 'replace'))
      file.close()
...
class Stream:
  ...
  def resolve(self):
    ...
    xbmc.log("Resolving stream from: %s" % self.player_url)
    res = requests.get(self.player_url, headers=headers)
    #if 'vmro' in self.player_url:
    #  xbmc.log(res.text)
    m = re.compile('(http.*m3u.*?)[\s\'"]+').findall(res.text)
    xbmc.log('Found %s m3u matches in source ' % len(m))
